...
begin
  ...
  FSelected := Value;
  if Value <> -1 then MakeVisible(FSelected);
  if Assigned(FOnSelect) then begin
    if IsVisible(FSelected) then
      FOnSelect(Self,FSelected,OldSelected) else
...
begin
  ...
  if Message.NMHdr^.code = NM_FIREVIEWCHANGE then begin
    ...
    if (nmh.bEvent = FVCN_GETINFO) and (nmh.bAction = FVCA_NONE) then begin
      RichItem := FRichCache.GetItemByHandle(Message.NMHdr^.hwndFrom);
      if not Assigned(RichItem) then exit;
...
begin
  ...
  if DoChanges then GridUpdate([guOptions]);
  EndUpdate;
end;
