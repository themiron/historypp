...
uses
  Windows,
  SysUtils,
  m_globaldefs in '..\inc\m_globaldefs.pas',
...
var
  pszVersion: array[0..55] of AnsiChar;
begin
...
begin
  ...
  if SmileyAddEnabled    then HookSmAddChanged := PluginLink.HookEvent(ME_SMILEYADD_OPTIONSCHANGED,OnSmAddSettingsChanged);
  if IcoLibEnabled       then HookIcon2Changed := PluginLink.HookEvent(ME_SKIN2_ICONSCHANGED,OnIcon2Changed);
  if FontServiceEnabled  then HookFSChanged := PluginLink.HookEvent(ME_FONT_RELOAD,OnFSChanged);
  if MetaContactsEnabled then HookMetaDefaultChanged := PluginLink.HookEvent(ME_MC_DEFAULTTCHANGED,OnMetaDefaultChanged);
...
var
  ttb: TTBButtonV2;
begin
  if Boolean(PluginLink.ServiceExists(MS_TTB_ADDBUTTON)) then
  begin
...
begin
  ...
  menuitem.hIcon := hppIcons[HPP_ICON_CONTACTHISTORY].handle;
  PluginLink.CallService(MS_CLIST_MODIFYMENUITEM, MenuHandles[miContact].Handle, LPARAM(@menuItem));
  PluginLink.CallService(MS_CLIST_MODIFYMENUITEM, MenuHandles[miSystem].Handle, LPARAM(@menuItem));
  menuitem.hIcon := hppIcons[HPP_ICON_GLOBALSEARCH].handle;
  PluginLink.CallService(MS_CLIST_MODIFYMENUITEM, MenuHandles[miSearch].Handle, LPARAM(@menuItem));
  menuitem.hIcon := hppIcons[HPP_ICON_TOOL_DELETEALL].handle;
  PluginLink.CallService(MS_CLIST_MODIFYMENUITEM, MenuHandles[miEmpty].Handle, LPARAM(@menuItem));
  PluginLink.CallService(MS_CLIST_MODIFYMENUITEM, MenuHandles[miSysEmpty].Handle, LPARAM(@menuItem));
end;
...
begin
  Result := 0;
  count := PluginLink.CallService(MS_DB_EVENT_GETCOUNT, awParam, 0);
  ...
  begin
    ...
    PluginLink.CallService(MS_CLIST_MODIFYMENUITEM, MenuHandles[miEmpty].Handle,
      lParam(@menuitem));
    if ShowHistoryCount then
    begin
      menuitem.flags := menuitem.flags or CMIM_NAME;
      menuitem.pszName :=
        pAnsiChar(AnsiString(Format(AnsiString('%s [%u]'), [MenuHandles[miContact].Name,
        count])));
    end
    ...
    res := PluginLink.CallService(MS_CLIST_MODIFYMENUITEM, MenuHandles[miContact].Handle,
      lParam(@menuitem));
    if res = 0 then
      MenuCount := count;
