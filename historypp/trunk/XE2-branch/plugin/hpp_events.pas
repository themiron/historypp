...
begin
  ...
  etd := Pointer(PluginLink.CallService(MS_DB_EVENT_GETTYPE, WPARAM(PAnsiChar(Hi.Module)),
    LPARAM(Hi.EventType)));
  if etd = nil then
  begin
...
begin
  ...
  if msglen > EventInfo.cbBlob then
  ...
  begin
    lenW := 0;
    if EventInfo.cbBlob >= msglen * SizeOf(Char) then
    begin
      msgW := PChar(msgA + msglen);
      for i := 0 to ((EventInfo.cbBlob - msglen) div SizeOf(Char)) - 1 do
        if msgW[i] = #0 then
        begin
          lenW := i;
          break;
...
begin
  BytePos := 0;
  Contacts := '';
  While BytePos < EventInfo.cbBlob do
  begin
...
begin
  ...
  begin
    lenW := 0;
    if EventInfo.cbBlob >= msglen * SizeOf(WideChar) then
    begin
