...
procedure RegisterFont(Name:PAnsiChar; Order:integer; defFont:FontSettings);
var
  fid: FontID;
begin
...
begin
  ...
  try
    ...
    begin
      CountIconsDll := ExtractIconExW(PWideChar(hppIconPack), -1, hIcon(nil^), hIcon(nil^), 0);
      NeedIcons := Min(Length(hppIcons), CountIconsDll);
      if NeedIcons > 0 then
      begin
        SetLength(SmallIcons, NeedIcons);
        CountIconsDll := ExtractIconExW(PWideChar(hppIconPack), 0, hIcon(nil^), SmallIcons[0],
          NeedIcons);
...
begin
  ...
  begin
    ...
    F.Height := size;
    fs := [];
    if (hppFontItems[Order].style and DBFONTF_BOLD) > 0 then
      include(fs, fsBold);
    F.style := fs;
...
begin
  ...
  begin
    ...
    sid.pszDefaultFile := PAnsiChar(AnsiString(hppIconPack)); // !!
    ...
    begin
      ...
      if hppIconsDefs[i].group = nil then
        sid.pszSection := Translate(hppName { TRANSLATE-IGNORE } )
      else
        sid.pszSection := PAnsiChar(Translate(hppName) { TRANSLATE-IGNORE } + '/' +
          Translate(hppIconsDefs[i].group) { TRANSLATE-IGNORE } );
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        sid.pszDescription :=
          Translate(PAnsiChar(WideToAnsiString(EventRecords[mt].name, hppCodepage))
          { TRANSLATE-IGNORE } );
        sid.pszSection := PAnsiChar(Translate(hppName) { TRANSLATE-IGNORE } + '/' +
          Translate('Events') { TRANSLATE-IGNORE } );
    ...
    begin
      ...
      begin
        if hppFontItems[i].nameColor = '' then
          RegisterColor(Translate(hppFontItems[i].name), i,
            hppFontItems[i].back { TRANSLATE-IGNORE } )
        else
          RegisterColor(Translate(hppFontItems[i].nameColor), i,
            hppFontItems[i].back { TRANSLATE-IGNORE } );
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    SaveFormats[sf].Filter := Format('%s (%s)|%s',
      [TranslateUnicodeString(SaveFormatsDef[sf].Filter { TRANSLATE-IGNORE } ),
