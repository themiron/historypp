...
type
  ...
  protected
    ...
    procedure GridTranslateTime(Sender: TObject; Time: Cardinal; var Text: String);
...
begin
  ...
  pmGrid.Items.Add(NewItem('Text Formatting', TextToShortCut('Ctrl+P'), False, True,
    OnTextFormattingClick, 0, 'pmTextFormatting'));
  pmGrid.Items.Add(NewItem('-', 0, False, True, nil, 0, 'pmN3'));
  pmGrid.Items.Add(NewItem('&Reply Quoted', TextToShortCut('Ctrl+R'), False, True,
    OnReplyQuotedClick, 0, 'pmReplyQuoted'));
  pmGrid.Items.Add(NewItem('Set &Bookmark', TextToShortCut('Ctrl+B'), False, True,
    OnBookmarkClick, 0, 'pmBookmark'));
  pmGrid.Items.Add(NewItem('-', 0, False, True, nil, 0, 'pmN4'));
  pmGrid.Items.Add(NewItem('&Save Selected...', TextToShortCut('Ctrl+S'), False, True,
    OnSaveSelectedClick, 0, 'pmSaveSelected'));
  pmGrid.Items.Add(NewItem('-', 0, False, True, nil, 0, 'pmN5'));
  pmGrid.Items.Add(NewSubMenu('&File Actions', 0, 'pmFileActions',
    [NewItem('&Browse Received Files', 0, False, True, OnBrowseReceivedFilesClick, 0,
    'pmBrowseReceivedFiles'), NewItem('&Open file folder', 0, False, True,
    OnOpenFileFolderClick, 0, 'pmOpenFileFolder'), NewItem('-', 0, False, True, nil, 0, 'pmN7'),
  ...
  pmGrid.Items.Add(NewSubMenu('Text direction', 0, 'pmBidiMode',
    [RadioItem(True, NewItem('Log default', 0, True, True, OnBidiModeLogClick, 0,
    'pmBidiModeLog')), RadioItem(True, NewItem('History default', 0, False, True,
    OnBidiModeHistoryClick, 0, 'pmBidiModeHistory'))], True));
  pmGrid.Items.Add(NewSubMenu('ANSI Encoding', 0, 'pmCodepage',
    [RadioItem(True, NewItem('Log default', 0, True, True, OnCodepageLogClick, 0,
    'pmCodepageLog')), RadioItem(True, NewItem('History default', 0, False, True,
    OnCodepageHistoryClick, 0, 'pmCodepageHistory'))], True));
  ...
  pmLink.Items.Add(NewItem('Open Link in New &Window', 0, False, True, OnOpenLinkNWClick, 0,
    'pmOpenLinkNW'));
...
begin
  ...
  begin
    if (not Item.IsRead) then
      PluginLink.CallService(MS_DB_EVENT_MARKREAD, Items[Index].hContact,
...
procedure TExternalGrid.GridTranslateTime(Sender: TObject; Time: Cardinal; var Text: String);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Item.Size := Cardinal(DBEventInfo.cbSize) + DBEventInfo.cbBlob;
    end
    else
    begin
...
begin
  ...
  if Boolean(PluginLink.ServiceExists(MS_SPEAK_SAY_W)) then
    PluginLink.CallService(MS_SPEAK_SAY_W, hContact, LParam(PChar(mesW)))
  else
  begin
    mesA := WideToAnsiString(mesW, Items[Grid.Selected].Codepage);
    PluginLink.CallService(MS_SPEAK_SAY_A, hContact, LParam(PAnsiChar(mesA)));
  end;
end;
