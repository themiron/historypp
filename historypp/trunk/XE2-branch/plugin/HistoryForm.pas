...
begin
  ...
  begin
    if not(tool[i] is THppSpeedButton) then
      tool[i].Parent := nil;
    tool[i].Left := -3;
    tool[i].Visible := True;
    if not(tool[i] is THppSpeedButton) then
...
procedure sButtonIcon(var sb: THppSpeedButton; Icon: HICON);
begin
  with sb.Glyph do
  begin
    Width := 16;
...
begin
  ...
  begin
    ...
    else if mmToolbar.Items[i].Owner is THppSpeedButton then
    begin
      TMenuItem(mmToolbar.Items[i]).Caption := THppSpeedButton(mmToolbar.Items[i].Owner).Hint;
...
begin
  case Message.Msg of
    ...
    WM_MEASUREITEM:
      ...
      begin
        Message.Result := PluginLink.CallService(MS_CLIST_MENUMEASUREITEM, Message.wParam,
          Message.lParam);
        if Message.Result <> 0 then
          Exit;
      end;
    WM_DRAWITEM:
      if Self.UserMenu <> 0 then
      begin
        Message.Result := PluginLink.CallService(MS_CLIST_MENUDRAWITEM, Message.wParam,
          Message.lParam);
        if Message.Result <> 0 then
          Exit;
      end;
