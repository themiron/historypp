          ...
          bbUrl:
            begin
              ...
              if doColorBBCodes then
                ...
                n := 0;
              newCode := StrLFmt(fmt_buffer, MAX_FMTBUF, bbCodes[i, bbStart].rtf,
                [PAnsiChar(code), n]);
            end;
{$ENDIF}
          bbImage:
  ...
  for i := 0 to High(bbCodes) do
    ...
    begin
      if bbCodes[i, bbClass].bbtype = bbSimple then
        WideStream := StringReplace(WideStream, bbCodes[i, bbClass].prefix.wide, '',
          [rfReplaceAll])
      else
        repeat
...
begin
  ...
  hBmp := PluginLink.CallService(MS_UTILS_LOADBITMAP, 0, LPARAM(@Link[1]));
  if hBmp <> 0 then
  begin
...
initialization
  ...
  begin
    rtf_ctable_text := rtf_ctable_text + AnsiString(format('\red%d\green%d\blue%d;',
      [rtf_ctable[i].col and $FF, (rtf_ctable[i].col shr 8) and $FF,
